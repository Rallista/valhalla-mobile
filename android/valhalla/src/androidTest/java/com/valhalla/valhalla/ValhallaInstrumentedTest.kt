...
class ValhallaInstrumentedTest {
    ...
    fun testNoConfigPath() {
        val valhalla = Valhalla("invalid.json")

        val request = "{\"locations\":[{\"lat\":45.843812,\"lon\":-123.768205},{\"lat\":45.869701,\"lon\":-123.766121}],\"costing\":\"auto\",\"units\":\"miles\"}"
    ...
    fun testNoSuitableEdges() {
        val valhalla = Valhalla(configPath)
    ...
    fun testSuccessfulRoute() {
        ...
        val request = "{\"locations\":[{\"lat\":38.429719,\"lon\":-108.827425},{\"lat\":38.4604331,\"lon\":-108.8817009}],\"costing\":\"auto\",\"units\":\"miles\"}"
        val response = valhalla.route(request)

        val expectedResponse = TestFileUtils.getExpectedResponse(InstrumentationRegistry.getInstrumentation().targetContext)
