...
Both platforms carry the same surface. Where the two differ, it is in what the language makes
natural rather than in what the library can do.

| | Android | iOS |
| --- | :-: | :-: |
| Build from a config object | ✅ | ✅ |
| Build from a config file on disk | ✅ | ✅ |
| Bundled valhalla defaults | ✅ | ✅ |
| Tiles from a tarball or a directory | ✅ | ✅ |
| Elevation tiles | ✅ | ✅ |
| Tiles downloaded on demand (`tile_url`) | ✅ | ✅ |
| Parse an existing config JSON | ✅ | ✅ |
| A config file per instance | ✅ | ✅ |
| Raw request methods for every action | ✅ | ✅ |
| Typed errors | `ValhallaException` | `ValhallaError` |
| Explicit release | `close()` | `close()` |
| Interface for test doubles | – | `ValhallaProviding` |

Tiles are downloaded through the platform's own HTTP stack — `HttpURLConnection` on Android,
`NSURLConnection` on iOS — so neither adds a networking dependency to your app. Android needs the
consuming app to declare `android.permission.INTERNET`; this library does not declare it, so that
offline-only consumers do not inherit it.

The default config both platforms start from is generated by valhalla's own
`scripts/valhalla_build_config` from the pinned submodule, and the identical bytes are bundled for
each. See [docs/src/bumping-valhalla.md](docs/src/bumping-valhalla.md).
